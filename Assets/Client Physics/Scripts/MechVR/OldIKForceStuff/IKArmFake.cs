...
public class IKArmFake
{
	...
	public IKArmFake(GameObject c0, GameObject c1, GameObject c2, GameObject c3, GameObject target, GameObject elbowTarget, float maxForce, float defaultSpring, float defaultDampening)
	{
		this.c0 = c0;
		this.c1 = c1;
		this.c2 = c2;
	...
	public void UpdateTargetRotsGlobal()
	{
		...
		Debug.DrawRay(c0.transform.position, tPosition);
		...
		offsetQuat = Quaternion.AngleAxis(elbowAngle, Vector3.Cross(tmp2 * tPosition, tPosition));

		c1.transform.localPosition = offsetQuat * Quaternion.FromToRotation(Vector3.right, tPosition) * Vector3.right * length1 + c0.transform.localPosition;
		c1.transform.localRotation = Quaternion.FromToRotation(Vector3.right, c1.transform.localPosition - c0.transform.localPosition);

		c2.transform.localPosition = Quaternion.FromToRotation(Vector3.right, tPosition - c1.transform.localPosition + c0.transform.localPosition) * Vector3.right * length2 + c1.transform.localPosition;
		c2.transform.localRotation = Quaternion.FromToRotation(Vector3.right, c2.transform.localPosition - c1.transform.localPosition);
